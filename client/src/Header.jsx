...
import { useContinent, AnywhereOptions } from "./pages/Components/AnywhereOptions"
export default function Header() {
    const {user} = useContext(UserContext)
    const {showOptions, toggleOptions, selectedOption, setContinent} = useContinent()
    return (
    <header>
      <div className="flex justify-between">
        ...
        <span className="font-bold text-xl" onClick={() => setContinent('Anywhere')}>padwhen</span>
        </Link>
        ...
        <Link to={user?'/account':'/login'} className='hover:drop-shadow-xl flex gap-2 border border-gray-300 rounded-full py-2 px-3'>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
